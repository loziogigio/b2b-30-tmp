...
import { transformOrder, TransformedOrder, RawOrderResponse } from '@utils/transform/b2b-order';
...
export async function fetchOrderDetails(params: OrderParams): Promise<TransformedOrder> {
    const payload = toErpPayload(params);
    const res = await post<RawOrderResponse>(API_ENDPOINTS_B2B.GET_ORDER_DETAIL, payload);
