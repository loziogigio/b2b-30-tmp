...
export default function ProductCardWrapper({
  ...
  // Get custom styles
  const customStyle = globalCardStyle
    ? getCardStyleCSS(globalCardStyle)
    : undefined;

  const hoverClass = globalCardStyle
    ? getCardHoverClass(globalCardStyle.hoverEffect)
    : '';
  ...
  const combinedClassName = useMemo(
    () =>
      [
        ...
        hoverBackgroundColor ? 'transition-colors duration-200' : undefined,
      ]
        .filter(Boolean)
        .join(' '),
