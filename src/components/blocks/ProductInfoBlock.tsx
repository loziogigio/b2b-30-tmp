...
export function ProductInfoBlock({ config, productData }: ProductInfoBlockProps) {
  if (!productData) {
    return null;
  }
  ...
  return (
    <div className={`product-info-block layout-${layout}`}>
      ...
      <div className="product-info-content">
        <p className="text-sm text-gray-500">Product Info Block (to be integrated with existing component)</p>
        <pre className="text-xs">{JSON.stringify({ showImages, showPrice, showDescription }, null, 2)}</pre>
      </div>
    </div>
  );
}
