...
const shadowMap: Record<Exclude<ProductCardStyle['shadowSize'], 'none'>, string> = {
...
const hoverShadowMap: Record<Exclude<ProductCardStyle['hoverShadowSize'], undefined>, string> = {
  sm: shadowMap.sm,
  md: shadowMap.md,
  lg: shadowMap.lg,
  xl: shadowMap.xl,
...
export function computeCardStyle(style: ProductCardStyle): CSSProperties {
  ...
  return {
    ...
    boxShadow:
      style.shadowSize !== 'none' ? shadowMap[style.shadowSize as Exclude<ProductCardStyle['shadowSize'], 'none'>] : 'none',
