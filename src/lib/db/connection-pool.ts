...
const MAX_CONNECTIONS = parseInt(process.env.POOL_MAX_CONNECTIONS ?? '50');
const PER_DB_POOL_SIZE = parseInt(process.env.POOL_PER_DB_SIZE ?? '10');
const TTL_MS = parseInt(process.env.POOL_TTL_MS ?? '1800000'); // 30 min
...
export async function getPooledConnection(
  ...
  if (cached && cached.connection.readyState === 1) {
    console.log(`[Pool] Cache hit: ${dbName}`);
