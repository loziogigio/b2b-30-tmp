...
export const ADDRESS_STATE_COOKIE = 'b2b_address_state';
export const ADDRESS_STATE_COOKIE_MAX_AGE = 60 * 60 * 24 * 30; // 30 days
...
export const normalizeAttributes = (
  ...
  const entries =
    input instanceof Map
      ...
      : Array.from(Object.entries(input));
...
export const contextHasData = (context?: PageContext | null) => {
  ...
  return Boolean(
    normalizeValue(context.campaign) ||
      ...
      normalizeValue(context.addressState) ||
      normalizeAttributes(context.attributes),
  );
};

export const mergeContexts = (
  ...contexts: Array<PageContext | null | undefined>
): PageContext | null => {
  ...
  contexts.forEach((ctx) => {
    ...
    if (ctx.addressState) merged.addressState = ctx.addressState;
    if (ctx.attributes) {
...
export const parseContextCookie = (
  value?: string | null,
): PageContext | null => {
...
export const ensureLanguageAttribute = (
  ...
  const normalizedLang = languages.includes(langValue)
    ? langValue
    : fallbackLng;
...
export const extractAttributesFromUserAgent = (
