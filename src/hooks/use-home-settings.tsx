...
import type { CompanyBranding, ProductCardStyle } from '@/lib/home-settings/types';
...
export function useHomeSettings() {
  ...
  const [fallbackSettings, setFallbackSettings] = useState<HomeSettings | null>(contextValue?.settings ?? DEFAULT_HOME_SETTINGS);
  const [fallbackLoading, setFallbackLoading] = useState(contextValue ? contextValue.isLoading : false);
  const [fallbackError, setFallbackError] = useState<string | null>(contextValue?.error ?? null);
...
export type { HomeSettings, CompanyBranding, ProductCardStyle } from '@/lib/home-settings/types';

/**
 * Convert ProductCardStyle to CSS styles
 */
export function getCardStyleCSS(cardStyle: ProductCardStyle): React.CSSProperties {
  ...
  return {
    ...
    boxShadow: cardStyle.shadowSize !== "none" ? shadowMap[cardStyle.shadowSize] : "none",
 ...
 */
export function getCardHoverClass(hoverEffect: ProductCardStyle["hoverEffect"]): string {
  switch (hoverEffect) {
    case "lift":
