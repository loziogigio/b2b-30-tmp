...
type RawHomeBrandItem = {
  ...
  export function transformPromoBanner(raw: RawPromoBannerItem[]): BannerCardItem[] {
    return raw
      .filter((item) => item.b2b === 1 && item.status === 'Published')
      .sort((a, b) => a.order - b.order)
      .map((item, index) => ({
  ...
  export function transformHomeBrand(rawBrands: RawHomeBrandItem[]): BannerCardItem[] {
    return rawBrands
      .filter((item) => item.b2b === 1 && item.status === 'Published')
      .sort((a, b) => a.order - b.order)
      .map((item, index) => ({
...
export function transformSliderTop(rawSlider: SliderRawItem[]): BannerCardItem[] {
  return rawSlider
    .filter((item) => item.b2b === 1 && item.status === 'Published')
