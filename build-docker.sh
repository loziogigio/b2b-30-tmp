...
echo "    NEXT_PUBLIC_THEME=${NEXT_PUBLIC_THEME}"
...
docker build \
  ...
  --build-arg NEXT_PUBLIC_MODAL_FULL_WIDTH="${NEXT_PUBLIC_MODAL_FULL_WIDTH:-false}" \
...
if [[ "$IS_MULTI_TENANT" == "true" ]]; then
    ...
    echo "    -e TENANTS_DB=vinc-admin \\"
    echo "    -e SSO_API_URL=https://cs.vendereincloud.it \\"
    echo "    -e SSO_CLIENT_ID=vinc-b2b \\"
    echo "    -e SSO_CLIENT_SECRET=your-secret \\"
    echo "    ${IMAGE_NAME}:${VERSION}"
    echo "  MONGO_URL/TENANTS_MONGO_URL must point to the same MongoDB cluster used by Commerce Suite."
    echo ""
    echo "  Optional — add for push cache invalidation (requires Redis):"
    echo "    -e REDIS_HOST=<host> -e REDIS_PORT=6379"
    echo ""
fi
