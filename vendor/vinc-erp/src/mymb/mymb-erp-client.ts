...
export class MyMbErpClient implements ErpClient {
  ...
  async getMultiplePrices(input: PriceQuery): Promise<Record<string, MyMbPriceEntry>> {
    ...
    if (erpChannel) (body as Record<string, unknown>).canale = erpChannel;
  ...
  async getLatestOrderByItem(input: {
    ...
    const data = await this.request<any>(
      MYMB_ENDPOINTS.GET_ULTIMO_ORDINATO_CLIENTE_X_ARTICOLO,
      {
        method: 'GET',
        params: {
          CodiceInternoCliente: input.customerCode,
   ...
   */
  async getCustomerAddressAgents(
    customerCode: string,
  ): Promise<CustomerAddressAgent[] | null> {
    try {
      const data = await this.request<any>(
        MYMB_ENDPOINTS.GET_INDIRIZZI_CLIENTE,
        { method: 'GET', params: { CodiceInternoCliente: customerCode } },
      );
      const result = data?.GetIndirizziClienteResult;
      if (!result || result.ReturnCode !== 0) {
        // `Message` carries ERP server paths — log the code, never the text.
        console.warn(
          '[MyMB] GetIndirizziCliente ReturnCode',
          result?.ReturnCode,
        );
        return null;
      }
      const list = Array.isArray(result.ListaIndirizzi)
        ...
        : [];
      return list.map(buildCustomerAddressAgent);
    } catch (err) {
      console.warn(
        '[MyMB] GetIndirizziCliente failed:',
